...
class CarModel:
    ...
    def update_kinematic_state(self, v, d_ref, dt):
        self.x = self.x + v * np.sin(self.theta) * dt
        self.y = self.y + v * np.cos(self.theta) * dt
        self.theta = self.theta + dt * v / self.wheelbase * np.tan(d_ref)
...
class TrackSim:
    ...
    def step(self, action):
        ...
        for i in range(frequency_ratio):
            ...
            self.car.update_kinematic_state(v, d_ref, self.timestep)
...
class AgentMPC:
    def __init__(self):
        ...
        self.d_hist     = np.zeros((self.Nsim+1, self.N+1))
    ...
    def init_ocp(self):
        ...
        ocp.set_der(self.sth,      self.v / self.L * tan(self.d) * cos(arctan2(self.sth, self.cth)))
        ocp.set_der(self.cth,     - self.v / self.L * tan(self.d) * sin(arctan2(self.sth, self.cth)))
        ...
        th_dot_lim = 0.4
        ocp.subject_to( -th_dot_lim <= (self.d <= th_dot_lim) )
    ...
    def first_solve(self):
        ...
        t_sol, d_sol = sol.sample(self.d, grid='control')
        ...
        current_U = vertcat(v_sol[0], d_sol[0])
